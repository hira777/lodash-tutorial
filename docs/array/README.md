...
配列の末尾の要素から反復処理を実行し、反復処理が`false`を返すまでの要素を削除した配列を返す。
...
配列の先頭の要素から反復処理を実行し、`false`を返すまでの要素を削除した配列を返す。
...
全ての配列に対して反復処理を実行し、その結果が全ての配列に含まれる場合、結果を計算するために利用した要素を格納した配列を返す。
