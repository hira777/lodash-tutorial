...
複数の配列から「戻り値が重複する反復処理の引数に渡している要素」を取り除いた配列を返す。

言葉の説明だと非常にわかりづらいため、サンプルコードを見た方が理解しやすいと思う。

```js
// [2.1], [1, 2.3] に`Math.floor`を実行すると
// [2], [1, 2] になる。戻り値が重複するのは`2`であり
// この`2`を計算するために`Math.floor`に渡していた`2.3`が削除される（引数の中で一番最初に現れる`2.1`が残る）。
